...
Expected<Model, std::string> loadModel(AlpacaCoreSwift::ModelAssetDesc& desc, DictRef params, ProgressCallbackData progressCbData) {
    ac::local::ModelAssetDesc modelDesc;
    modelDesc.type = desc.getType();
    modelDesc.name = desc.getName();

    for (auto asset : desc.getAssets()) {
        ac::local::ModelAssetDesc::AssetInfo assetInfo;
    ...
    try {
        if (progressCbData.m_cb) {
            return Model(ac::local::Lib::loadModel(modelDesc, params.getDict(), [&](std::string_view tag, float progress) {
                progressCbData.m_cb(progressCbData.m_context, tag.data(), tag.data() + tag.size(), progress);
                return true;
            }));
        }
        else {
            return Model(ac::local::Lib::loadModel(modelDesc, params.getDict()));
        }
    }
